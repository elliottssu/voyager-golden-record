...
function nextLine(channel) {
  ...
  channel.offset += 300;
  var maxInterimageSamples = 10000; /*Not actually 10000*/
  var max = 0;
  var lookahead = 850;
  ...
  for (var i = -100; i < maxInterimageSamples; i++) {
...
function startDisplayingChannel(channel) {
  setInterval(function () {
    ...
    if (debug) {
      ...
      if (samplesScannedBeforePulse > AVG_SAMPLES_PER_LINE + 80 ||
...
window.onload = function () {
  loadSound("https://s1.ogww.com/lab/spacecraft/golden-record/audio-image.mp3", onloadCallback, loadingProgress);
};
